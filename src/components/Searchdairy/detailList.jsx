...
const DetailList = ({ num }) => {
  ...
  const handleDetailBoxClick = (data) => {
    //클릭한 세부요소 데이터 저장
    setSelectedBox(data);
  };
            ...
            <table>
              <tr>
                <td style={{ width: "50px" }} rowSpan={4}>
                  ...
                  <span className={styles.detailnum2}>{`${index + 1}`}</span>
                </td>
  ...
  return (
    <div style={{ marginLeft: "100px" }}>
      ...
      <div className={styles.hashtags}>#힐링 #가족여행 #맛집탐방</div>
      <hr className={styles.line}></hr>
      {/* JSX 내용 작성 */}
      <div>
        <div className={styles.divscroll}>{generateDay()}</div>
      </div>
      {selectedBox && (
        <Modal
          data={selectedBox}
